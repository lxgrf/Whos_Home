...
def wait(duration): sleep.sleep(duration)
...
### MAIN LOOP ###      
while True:
              if in_between(datetime.now().time()): #Check current time is between the functional hours
                            if ping(ipA): countA = cooldown # Ping successful, reset count
                            elif countA > 0: countA -= 1 #Ping unsuccessful, decrement count
 
                            if ping(ipL): countL = cooldown
